...
const dynamicResolve: QuartzPlugin["dynamicResolve"] = async ({ name, moduleId, config }) => {
    ...
    const loadPromise = (async () => {
        ...
        if (rawCode.length < 500 && !rawCode.includes("export") && !rawCode.includes("module.exports")) {
            tidalModules[path] = {};
            return {};
        }

        const modStart = performance.now();
        const module = await quartz(rawCode, {
            plugins: [
                {
    ...
    const loadPromise = (async () => {
        ...
        const module = await quartz(
            scriptContent,
            {
                ...
                plugins: [
                    {
                        // Transform is now done on the native side — skip it
                        transform({ code }) {
                            return code;
                        },
                        dynamicResolve,
...
const { resolveReact } = await import("./modules");
resolveReact();
